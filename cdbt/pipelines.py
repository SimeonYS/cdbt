...
class CcdbtPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('cdbt.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `cdbt`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM cdbt WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO cdbt (date, title, link, content)"
